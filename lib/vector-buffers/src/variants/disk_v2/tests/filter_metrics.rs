...
use crate::{
    ...
    topology::channel::{limited, BufferSender, SenderAdapter},
...
async fn unencodable_item_overflows_intact_when_base_has_room() {
    ...
    with_temp_dir(|dir| {
        ...
        async move {
            ...
            let (overflow_tx, mut overflow_rx) = limited(100);
...
async fn unencodable_item_overflows_intact_when_base_is_full() {
    ...
    let (base_tx, _base_rx) = limited::<FilterableBatch>(1);
    let (overflow_tx, mut overflow_rx) = limited(100);
...
async fn unencodable_item_stays_in_base_when_base_has_no_encoding_constraint() {
    ...
    let (base_tx, mut base_rx) = limited::<FilterableBatch>(100);
    let (overflow_tx, mut overflow_rx) = limited(100);
