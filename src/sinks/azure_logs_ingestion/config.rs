...
use crate::sinks::prelude::TlsSettings;
use crate::sinks::prelude::Transformer;
use crate::sinks::HealthcheckError;
use crate::sinks::util::http::HttpService;
use crate::sinks::prelude::ServiceBuilder;
...
use crate::{codecs::{EncodingConfigWithFraming, SinkType}, http::{Auth, HttpClient, MaybeAuth}, config::{
    AcknowledgementsConfig, GenerateConfig, Input, SinkConfig,
    SinkContext,
}, sinks::{
    azure_logs_ingestion::{
        retry::AzureLogIngestionRetryLogic,
    },
    util::{
        http::RequestConfig,
        BatchConfig,
...
use crate::sinks::prelude::{ExternalResource, HttpResourceConfig, ResourceDirection, ValidatableComponent, ValidationConfiguration};

use crate::sinks::util::http::{HttpResponse};


/// Creates a `RetryLogic` for use with `HttpResponse`.
pub fn azure_response_retry_logic() -> AzureLogIngestionRetryLogic<
    impl Fn(&HttpResponse) -> StatusCode + Clone + Send + Sync + 'static,
    HttpResponse,
> {
    AzureLogIngestionRetryLogic::new(|req: &HttpResponse| req.http_response.status())
}
...
pub(super) struct AzureLogsIngestionSinkConfig {
    ...
    #[configurable(metadata(docs::examples = "${VECTOR_AZURE_APPLICATION_SECRET}"))]
    pub application_secret: SensitiveString,
...
impl AzureLogsIngestionSinkConfig {
...
impl GenerateConfig for AzureLogsIngestionSinkConfig {
...
impl SinkConfig for AzureLogsIngestionSinkConfig {
    async fn build(&self, cx: SinkContext) -> crate::Result<(VectorSink, Healthcheck)> {
        ...
        let service = ServiceBuilder::new()
            .settings(request_limits, azure_response_retry_logic())
            .service(service);
...
impl ValidatableComponent for AzureLogsIngestionSinkConfig {
    fn validation_configuration() -> ValidationConfiguration {
        ...
        let config = Self {
            ...
            application_secret: "${VECTOR_AZURE_APPLICATION_SECRET}".to_owned().into(),
        };
...
register_validatable_component!(AzureLogsIngestionSinkConfig);
